...
class E160_robot:
    ...
    def __init__(self, environment, address, robot_id):
        ...
        self.file_name = 'Log\Bot' + str(self.robot_id) + '_' + datetime.datetime.now().strftime('%y%b%d_%H%M%S') + '.txt'
        #
        self.make_headers()
    ...
    def update_control(self, range_measurements):
        ...
        if self.environment.control_mode == "MANUAL CONTROL MODE":
            ...
            theshold = 540
            buffer = 40 
            upperlimit = theshold - buffer
            lowerlimit = theshold + buffer
            leftSpeed = 20
            rightSpeed = 15
            if range_measurements[0] < upperlimit:
                R= -rightSpeed
                L= -leftSpeed
            elif range_measurements[0] > lowerlimit:
                R = rightSpeed
                L = leftSpeed
            else:
                print ('at 30 cm')
