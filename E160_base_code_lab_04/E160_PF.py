...
class E160_PF:
	...
	def Propagate(self, encoder_measurements, i):
        ...
        wheelDistanceL += random.gauss(0.0, self.odom_xy_sigma) # switch 1.0 with odometry noise 
        wheelDistanceR = (+ 2 * 3.14 * self.wheel_radius / self.encoder_resolution * (diffEncoder1)) + random.random(); # Positive since this is right wheel
        wheelDistanceR += random.gauss(0.0, self.odom_xy_sigma) # switch 1.0 with odometry noise 
        ...
        heading = theta + delta_theta + random.gauss(0.0, self.odom_heading_sigma)
        heading %= 2+math.pi
	...
	def CalculateWeight(self, sensor_readings, walls, particle):
        ...
        dist = FindMinWallDistance(particle, walls, sensor_readings[1]) # only looking at forward sensor
        prob = 1.0;
        prob *= self.Gaussian(dist, self.IR_sigma, sensor_readings[1])
